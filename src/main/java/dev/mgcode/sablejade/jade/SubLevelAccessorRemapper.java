package dev.mgcode.sablejade.jade;
...
public final class SubLevelAccessorRemapper {
    private static final String HONEY_GLUE_ENTITY_ID = "simulated:honey_glue";

    private SubLevelAccessorRemapper() {
    }

    public static void register(final IWailaClientRegistration registration) {
    ...
    private static boolean isHoneyGlueAccessor(final Accessor<?> accessor) {
        return accessor instanceof EntityAccessor entityAccessor
                && HONEY_GLUE_ENTITY_ID.equals(Objects
                        .requireNonNull(EntityType.getKey(entityAccessor.getEntity().getType())).toString());
    }
